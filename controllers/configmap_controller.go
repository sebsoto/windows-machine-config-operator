...
func (r *ConfigMapReconciler) parseInstances(configMapData map[string]string, nodes *core.NodeList) ([]*instances.InstanceInfo, error) {
	...
	for address, data := range configMapData {
		...
		// Check if the described instance has a node in the cluster
		var linkedNode *core.Node
		node, found := findNode(address, nodes)
		if found {
			linkedNode = node
		}

		instanceList = append(instanceList, instances.NewInstanceInfo(address, splitData[1], "", linkedNode))
	}
