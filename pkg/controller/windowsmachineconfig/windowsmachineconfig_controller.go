...
func (r *ReconcileWindowsMachineConfig) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.getCloudProvider(instance); err != nil {
		...
		r.statusMgr.conditionsToSet = []wmcapi.WindowsMachineConfigCondition{
	...
	degraded := newDegradedCondition(nodeReconcileErr)
	r.statusMgr.conditionsToSet = []wmcapi.WindowsMachineConfigCondition{
	...
	if nodeReconcileErr != nil {
		return reconcile.Result{}, fmt.Errorf("reconcile error: %v", nodeReconcileErr)
	}
	return reconcile.Result{}, nil
...
func (r *ReconcileWindowsMachineConfig) removeWorkerNode(vm types.WindowsVM) *reconcileError {
	...
	if err := r.cloudProvider.DestroyWindowsVM(id); err != nil {
		return &reconcileError{reason: wmcapi.VMTerminationFailureReason,
			err: errors.Wrapf(err, "error destroying VM with ID %s", id)}
	}
...
func (r *ReconcileWindowsMachineConfig) removeWorkerNodes(count int) (int, []*reconcileError) {
	...
	for i := 0; i < count; i++ {
		...
		if vm == nil {
			errs = append(errs, &reconcileError{reason: wmcapi.VMTerminationFailureReason,
				err: fmt.Errorf("expected VM and got a nil value")})
			continue
		}
...
func (r *ReconcileWindowsMachineConfig) addWorkerNode() (types.WindowsVM, *reconcileError) {
	...
	if err != nil {
		return nil, &reconcileError{reason: wmcapi.VMCreationFailureReason,
			err: errors.Wrap(err, "error creating windows VM")}
	}
	...
	if err := nc.Configure(); err != nil {
		...
		return nil, &reconcileError{reason: wmcapi.VMConfigurationFailureReason,
			err: errors.Wrap(err, "failed to configure Windows VM")}
	}
...
// reconcileError fulfills the built-in error interface and includes a reason for the degradation of the reconciler
type reconcileError struct {
	// reason is a computer readable reason for the error
	reason string
