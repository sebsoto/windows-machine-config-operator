...
metadata:
  ...
  name: ${NAMESPACE}
EOF
oc create secret -n ${NAMESPACE} generic smbcreds --from-literal username=USERNAME --from-literal password="PASSWORD"

## Deploy everything required for a CSI controller
#
# RBAC
oc apply -f - <<EOF
---
...
EOF
oc adm policy add-scc-to-user -n $NAMESPACE -z csi-smb-controller-sa privileged

# Deploy an SMB server
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: smb-server
          image: quay.io/ssoto/test:samba
          env:
            - name: PERMISSIONS
              value: "0777"
            - name: USERNAME
